...
function[F] = FSModel_1_Phase2_EPSchoice(parms, data)
...
subject_estimate = data(t, 7); % subject choice
    ...
    if     subject_estimate == 1 && val1ppt > val2ppt
        subject_estimate_pchoose1 = (1-epsilon_cong)   * (1./(1+exp(-(val1 - val2)))) + (epsilon_cong/2);
    elseif subject_estimate == 2 && val2ppt > val1ppt
        subject_estimate_pchoose1 = (1-epsilon_cong)   * (1./(1+exp(-(val1 - val2)))) + (epsilon_cong/2); 
    else 
        subject_estimate_pchoose1 = (1-epsilon_incong) * (1./(1+exp(-(val1 - val2)))) + (epsilon_incong/2);
    end
