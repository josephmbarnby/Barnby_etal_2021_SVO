...
%   their partner
%   a new alpha (5) and beta (6) to enter into phase 2
...
%% Model
function[F] = Model_10(parms, data)
...
param_alpha_v   = exp(alpha_v); % restrict the variance to be positive
param_beta_v    = exp(beta_v);  % restrict the variance to be positive

alpha_null = parms(5);
alpha_null_ = res*(1./(1+exp(-alpha_null))); % restrict alpha to between 0 and res
beta_null  = parms(6);
...
newpabg=normpdf(alpha_2,alpha_null_,param_alpha_v).*normpdf(beta_2,beta_null,param_beta_v);
...
subject_estimate = data(t, 7); % participant estimate from data
...
actual_choice = data(t, 8); % what did their partner 'choose'
