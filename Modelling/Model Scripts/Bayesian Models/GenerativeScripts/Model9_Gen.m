...
% GENERATIVE MODEL FOR MODEL 2
% Feed the model normally distributed parameters (m = 0, v = 7.5)
...
function[lik1, lik2, F, simA, simAFix, prob1, prob2, pptprob1, rew, u1, u2, congCount, alpha_marg, beta_marg, alpha_marg2, beta_marg2] = Model9_Gen(parms, data)
...
raw_epsc        = parms(5);
raw_epsic       = parms(6);
epsilon_cong    = 1./(1+exp(-raw_epsc))*2;
epsilon_incong  = 1./(1+exp(-raw_epsic))*2;
...
pchoose1_raw = 0*newpabg;  % initialize
cong = find(sign(val1(:)-val2(:))==sign(val1ppt - val2ppt)); % find congruent indices
icon = find(sign(val1(:)-val2(:))~=sign(val1ppt - val2ppt)); % find incongruent indices

pchoose1_raw(cong) = (1-epsilon_cong) * (1./(1+exp(-(val1(cong) - val2(cong))))) + (epsilon_cong/2);
%set the congruent ones with lapse rate epsilon_cong
pchoose1_raw(icon) = (1-epsilon_incong) * (1./(1+exp(-(val1(icon) - val2(icon))))) + (epsilon_incong/2);
%set the incongruent ones with lapse rate epsilon_incong
