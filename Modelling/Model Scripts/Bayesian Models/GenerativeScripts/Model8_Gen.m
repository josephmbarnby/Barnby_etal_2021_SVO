...
function[lik1, lik2, F, simA, simAFix, prob1, prob2, pptprob1, rew, u1, u2, congCount, alpha_marg, beta_marg, alpha_marg2, beta_marg2] = Model8_Gen(parms, data)
...
subject_estimate_pchoose1 = 0*newpabg;  % initialize
...
subject_estimate_pchoose1(cong) = (1-epsilon_cong) * (1./(1+exp(-(val1(cong) - val2(cong))))) + (epsilon_cong/2);
%set the congruent ones with lapse rate epsilon_cong
subject_estimate_pchoose1(icon) = (1-epsilon_incong) * (1./(1+exp(-(val1(icon) - val2(icon))))) + (epsilon_incong/2);
