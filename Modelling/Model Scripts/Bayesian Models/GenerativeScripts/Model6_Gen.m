...
function[lik1, lik2, F, simA, simAFix, prob1, prob2, pptprob1, rew, u1, u2, congCount, alpha_marg, beta_marg, alpha_marg2, beta_marg2] = Model6_Gen(parms, data)
...
kappa_raw       = parms(5); % whether participants are bias to choices than benefit themselves
kappa           = 1./(1+exp(-kappa_raw));
...
q2 = (1./(1+exp(-(val2 - val1)))) * exp(kappa * o2); %
    ...
    if (actual_choice==1)
        ...
        pchoose2 = q2./(q1+q2); % if the partner chose 2
    end
